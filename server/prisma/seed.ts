...
const seedIssues = (project: Project, users: User[]): Promise<Issue[]> => {
  const issues: Prisma.IssueCreateArgs["data"][] = [
    {
      ...
      assignees: { create: { assigneeId: users[0].id } },
    },
    {
    ...
    },
    {
      ...
      projectId: project.id,
      assignees: { create: { assigneeId: users[2].id } },
    },
    {
      summary: "Each issue can be assigned priority from lowest to highest.",
      type: IssueType.task,
      status: IssueStatus.selected_for_development,
    ...
    },
    {
      ...
      assignees: {
        createMany: {
          data: [{ assigneeId: users[1].id }, { assigneeId: users[2].id }],
        },
      },
    },
    {
    ...
    },
    {
      ...
      reporterId: users[0].id,
      projectId: project.id,
      assignees: { create: { assigneeId: users[1].id } },
    },
  ];
  return Promise.all(
...
const seedComments = (issues: Issue[], users: User[]): Promise<Comment[]> => {
  const comments: Prisma.CommentCreateArgs["data"][] = [
    {
